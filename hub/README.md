...
A single-binary Rust daemon (~6 MB) that turns a community chapter into a sovereign Web4 society — 7 roles, signed founding charter, append-only witnessed ledger, MCP HTTP server, admin CLI, Docker deployment. First deployment target: a pilot AIC chapter. Any community willing to operate as a Web4 society can use the same software.
...
- Not the AIC-Central Overlay layer (cross-chapter federation, global skill graph — V2)
