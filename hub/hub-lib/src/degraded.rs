...
//! signing capability returns (at unlock / signer recovery).
//!
//! Design properties:
...
impl DegradedLog {
    ...
    pub fn new(path: impl Into<PathBuf>) -> Self {
        Self { path: path.into(), lock: std::sync::Mutex::new(()) }
    }
    ...
    pub fn append(&self, source: DegradedSource, context: &str) {
        ...
        if let Err(e) = res {
            tracing::error!("degraded-log append failed at {}: {e}", self.path.display());
        }
    }
