...
- **Hardbound policy engine integration**: MVP encodes hub law as text (Sovereign-signed but not machine-enforced). Full policy enforcement is V2 — and requires resolving the Hardbound canonical Web4 alignment debt. **SHIPPED (hub-native):** hub law is now **machine-enforced** — the PolicyEntity gate evaluates every consequential request against the signed law and returns Allow/Deny/Escalate, on writes *and* reads. The *Hardbound* policy-engine integration specifically (the licensable enterprise layer) remains a separate V2+ track.
