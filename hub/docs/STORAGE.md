...
**Ignition / locked-shell boot (fail-closed).** On `hub serve`, the daemon tries to open the store with whatever key is available — which is *none*, since no passphrase sits on disk or in env. If the store opens (plaintext / NULL-keyed / fresh hub), it boots normally. If it fails closed (encrypted, no key), it boots a **LOCKED SHELL** that serves only the tier-0 allowlist — five paths, `/` among them, so a root health check still reads `200`; `hub unlock` ignites it at runtime, deriving the key and swapping in the real store in memory. An encrypted DB with no key never opens silently and is never corrupted — it fails closed.
