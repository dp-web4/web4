...
- **ROLE SKILLS** (dp, 2026-08-21): a role confers capability as well as reach — the skills its
  occupant should have loaded while filling it. Manifest items like any other, **each tagged with
  exactly one class**, so the flow rule governs them unchanged: a role may name a skill the
  occupant is not cleared for and the occupant does not receive it. Rotation carries them out;
  amendment re-passes them. The hub's half is the same as for paths — declare and tag them in the
  charter; hestia decides who receives them.
- **ROLE MEMBOTS**: *speculative, kept that way.* A role might also confer a memory cartridge.
  Strictly larger than skills, because a membot is retrieval whose returns are not fixed at
  conferral, so what reaches the occupant can change without re-passing the flow rule. Carried as
  an extension point in the home, not a decided concept. **Do not build against this.**
