...
**Negative results published** (v13.7, v24): the **v19 row-set census returns 0 silent drops** — every one of the 6 ids that left the ledger has a recorded closure or fold; and the **`hub/` M3 gate stays DECLINED**, its own trigger re-tested and negative on all three limbs — limbs 2 and 3 empty, limb 1 **fired lexically on the `security-framework` Rust crate in `hub/Cargo.lock` and is adjudicated inline** rather than published as a zero (§B′.3).
...
`web4-policy/` **M1-FAIL** — **and this cell's number is corrected too, by applying the rule the reviewer's block established.** The draft published *"0 predicate tokens."* Re-run at HEAD with the same predicate as `role.rs`, unrestricted filetypes, `--exclude-dir=target --exclude=Cargo.lock`: **2 matching lines in 1 file** — `web4-policy/src/lib.rs:399` and `:406`, both doc comments on `sha256_hex()`/`sha256_hex_of()`. **The verdict is unchanged and the reason is M2, not M1's count**: both are *uses* of a hash for the law digest (`LawAmended.new_law_sha256`), delegating to `web4_core::crypto::sha256_hex` — the tree defines no primitive and restates no suite value of its own. `web4-policy/` also changed **0 files in the window** (`git diff --name-only 47b270c3..HEAD -- web4-policy/` → 0), so it is frozen here regardless. **NEGATIVE, on measured evidence rather than on a zero.**

`web4-trust-core/` **M1-FAIL on subject matter** — guard unchanged from C296; re-tested at HEAD under the same predicate and exclusions, **genuinely 0 lines / 0 files**, and **0 files changed in the window**. It gained no §1/§2/§3 primitive.

> **Two false zeros, one root cause, and the second was found by the fix for the first.** The reviewer's block on §B′.3 named the class — *a matcher named after a document collides with the crates and modules sharing its name; publish the adjudication, not the zero*. Re-running **every** number this pass authored (not just the blocked one) turned up a second instance one section up, of the same shape and the same origin: a zero published for a scope the matcher was never actually run over. Neither changes a verdict; both changed what the verdict rests on. Recorded rather than quietly patched, because the count of instances is the evidence that this is a method defect and not a slip. → Key Adjudication #5.
...
|---|---|---|
| a `security-framework.md` citation | `grep -rl "security-framework" hub/` → **1: `hub/Cargo.lock`** — the `security-framework` v3.7.0 crate (Apple Security.framework bindings, transitive dep; also `security-framework-sys`, `:2340`/`:2413`/`:2422`/`:2426`). No spec citation. The precise matcher `grep -rlF "security-framework.md" hub/` → **0**. | NEGATIVE (lexical collision, adjudicated) |
| a suite-ID token | `grep -rlE "W4-BASE-1\|W4-FIPS-1\|W4-IOT-1" hub/` → **0** | NEGATIVE |
| a conformance claim | `grep -rliE "conformance\|conform" --include=*.rs --include=*.md --include=*.toml hub/` minus `hub/target/` → **0**; none in the 25 changed files | NEGATIVE |
...
**The first limb's number is an adjudication, not a zero — and that distinction is this section's own finding turned on itself.** §B′.2 one page up kills `role.rs` because a matcher grepping `rotat` would have admitted a **role-occupant** rotation; here a matcher grepping `security-framework` for a *document citation* admitted a **crate name**. Same error class, same document, one section apart. A published `0` would assert the trigger never fired; what happened is that it fired lexically and was reasoned past, which is the **stronger** negative — a scope exclusion resting on a gate that fired and was adjudicated is better evidence than one resting on a gate reported as untouched. Recorded this way so C376 inherits the adjudication rather than re-running the loose matcher, getting `1`, and either re-opening M3 spuriously or spending a cycle rediscovering the collision. *(Corrected on review — the draft published `0` here. The reviewer reproduced `1` three ways: GNU `grep -rl`, `git grep -l`, and with the cell's own `--exclude-dir=target`.)*
...
- **§B′**: set re-derived from the pre-registered predicate — **9 → 13 admitted**, 9 NEGATIVEs named, `web4-core/` re-established **by file**, `role.rs` **M1-FAIL** (0 predicate tokens; a "rotate" lexical collision), `hub/` M3 **re-tested NEGATIVE on all three limbs** (limb 1 **adjudicated**, not zero — the loose matcher hits the `security-framework` crate in `hub/Cargo.lock`).
...
5. **The pass committed the very error it charged, in the cell least able to afford it — and did not catch it itself.** §B′.2 kills `role.rs` on a lexical collision (`rotat` matching role-occupant rotation); §B′.3, one section later, published `0` for a limb whose printed matcher (`grep -rl "security-framework" hub/`) returns `1` — the `security-framework` v3.7.0 Rust crate in `hub/Cargo.lock`. **A matcher named after a document collides with the crates and modules that share its name**, and the collision was reported as absence rather than adjudicated. Two things make this worth a numbered entry rather than a quiet fix. First, **it landed in a scope-exclusion cell**: a gate reported as tested-and-empty looks more verified than a gate that fired and was reasoned past, so the defect made the exclusion look stronger than it was — the block-rather-than-record criterion. Second, **the pass's own self-correction machinery missed it**: v17's post-write re-run at a widened scope caught the B-7 undercount (§A.3) but was pointed only at the *carry's* matcher, never at the pass's *own gate cells*. The v17 re-run must cover every published number, including the ones the pass authored this fire. Corrected on review; the reviewer reproduced `1` three ways.

   **And applying the fix found a second instance the block did not name.** Re-running *every* number this pass authored — the discipline the entry above says was missing — turned up `web4-policy/`'s *"M1-FAIL (0 predicate tokens)"* in §B′.2, which returns **2 lines in `web4-policy/src/lib.rs`** under the same predicate used for `role.rs`. Same shape, same origin: a zero published for a scope the matcher was never run over. Verdict unchanged (both hits are *uses* of a hash delegating to `web4_core::crypto::sha256_hex` — an M2 failure, and the tree is frozen in the window besides), but **two instances in one document is the evidence that this is a method defect rather than a slip**, and it is why the corrected v17 rule is *re-run every published number*, not *re-run the carries*. → v29 rider 1, second half.

6. **A correctly closed row can still leave live work, and no census sees it.** `A-L3` was applied by #271, verified HELD at C68, and correctly dropped from the ledger. Its cross-track follow-through (`C32:105`, filed the next day) has been unnamed for 64 days. v19 counts rows that vanish **without** a disposition; this row vanished **with** one and still lost its tail. The row-set census is a necessary instrument and it is blind here by construction — which is why §B.3's third direction is not an optional extra.
...
- **`hub/` guard UNCHANGED and re-tested**: M3-DECLINED; trigger = a `security-framework.md` citation, a suite-ID token, or a conformance claim. Measured at HEAD over 25 changed hub files: limbs 2 and 3 **0**; limb 1 **1 — `hub/Cargo.lock`, the `security-framework` crate, adjudicated as a lexical collision, not a citation** (§B′.3). **Inherit the adjudication, not a bare zero**: the loose matcher will return `1` again at C376. Use `grep -rlF "security-framework.md" hub/` (→ 0) if a zero is wanted. C296-N4 (`envelope.rs` third canonicalization) carried forward in the C-M1 bundle, not re-litigated.
- **v17 CORRECTED — the post-write re-run covers EVERY published number, not just the carries'.** This pass's v17 re-run was pointed only at B-7's matcher; it caught that undercount and missed **two false zeros in the pass's own gate cells** (§B′.3 limb 1, §B′.2 `web4-policy/`) — both found afterwards, one by the reviewer and one by applying the reviewer's rule. At C376: re-run the gate cells too, and **when a trigger limb greps a filename, either match the extension (`grep -F "foo.md"`) or exclude lockfiles and manifests — a document-named matcher collides with crates and modules.**
...
1. **Check the filetype filter — and the alternation — of every carry's matcher before trusting its zero.** A carry whose grep says `--include=*.md` has been certifying the absence of nothing but markdown, however many trees it recurses. **Companion, in the other direction: a matcher named after a document will collide with the packages, crates and modules that share its name.** When a trigger limb greps a *filename*, either match the extension (`grep -F "foo.md"`) or exclude lockfiles and manifests — or adjudicate the hit inline and publish the adjudication instead of a zero. This is §B′.2's own rule (*grep the behaviour, not the vocabulary*) applied to the pass's own gate, and this pass failed it: §B′.3's first limb published `0` where the printed instrument returns `1`. **A gate cell is where an audit is most obliged to be exact — a scope exclusion resting on a tested-and-empty gate looks more verified than one resting on a gate that fired and was reasoned past.**
