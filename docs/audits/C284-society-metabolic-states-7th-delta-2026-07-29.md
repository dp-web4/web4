...
| `hub/hub-daemon/src/main.rs:1154` | ✗ (prints) | ✓ (operator CLI) | ✓ | **CONSUMER**, not a mirror → feeds **N1** |
| `web4-trust-core/src/bindings/wasm.rs:673` | ✗ | ✓ (published WASM/JS API) | ✓ | **CONSUMER**, not a mirror → feeds **N1** |
| `web4-trust-core/src/bindings/wasm.rs:628` | ✗ | ✓ (published WASM/JS API) | ✓ | **CONSUMER**, not a mirror → feeds **N1** |
...
### N1 — MEDIUM — C168-N1's mis-citation has four *shipped* consumer faces plus a committed typing, none recorded → operator + SDK/crate track, adjudicate **with** C168-N1
...
**Instrument:** `git grep -nE 'society\.state|inner\.state' 8bc3ef39 -- '*.rs'` → **7 sites**, of which 3 are tests (`web4-core/src/society.rs:321/341/344`, out of scope) and **4 are shipped consumers**:

| Site | Face | What it does |
|---|---|---|
| `hub/hub-daemon/src/admin.rs:282` | operator **web page** | renders `<dt>Metabolic state</dt><dd>{society.state}</dd>` — a human reading a running hub sees the spec's term over the wrong vocabulary |
| `hub/hub-daemon/src/main.rs:1154` | operator **CLI** | `QueryCommand::Chapter` prints `State: {:?}` — bare label, no "Metabolic" |
| `web4-trust-core/src/bindings/wasm.rs:673` | published **WASM/JS** getter | `WasmSociety::state()`, `inner: RustSociety = web4_core::society::Society`, doc-comment *"Current metabolic state as string"*, returning `"Genesis"\|"Bootstrap"\|"Operational"\|"Dormant"\|"Sunset"` to JavaScript |
| `web4-trust-core/src/bindings/wasm.rs:628` | published **WASM/JS** summary | `summary()` — `Reflect::set(&obj, &"state".into(), …)`, a **second** JS-visible face of the same enum |

**Two hub reads, not one. Two WASM faces, not one.** And the published TypeScript declaration a JS consumer compiles against is **committed in the tree**, not merely generated: `web4-trust-core/pkg/web4_trust_core.d.ts:308-310` ships *"Get a JSON summary of the society state." `summary(): any;`* and `:325-327` ships *"Current metabolic state as string." `readonly state: string;`*.

**Severity argued down, then up.** Down: **all four consumers are display or serialization** — HTML render, `println!`, `format!`, `Reflect::set`. **No code keys authority, law, or ATP off `society.state`** — every one of the 7 sites was checked — so there is no live authorization consequence today. Up to MEDIUM anyway on two grounds: (1) the composition hazard — SAL §3.6 defines "dormant states" as a *class of four metabolic states* (Sleep, Hibernation, Torpor, Estivation) and attaches an authority consequence ("dormant states SHOULD defer"), so "Metabolic state: **Dormant**" on an operator page is a term collision on a word that carries governance weight, with the two carried items (C168-N1 and B15/C58-B10) meeting for the first time; (2) the fix is monotonically getting more expensive — the rename is a **breaking change across two published WASM faces and a committed `.d.ts`**, not a comment edit.

The two arguments do **not** move together across the four sites, and flattening them would lose that: the term-collision argument (1) is *weaker* at `main.rs:1154`, which prints a bare `State:` with no "Metabolic" label, while the breaking-change-cost argument (2) is *stronger* at four surfaces plus a committed typing than at two. Recorded rather than averaged.

> **Correction, under review (this document, second commit).** This enumeration originally named **2** consumer faces (`admin.rs:282`, `wasm.rs:670`) and asserted `admin.rs:282` was *"the hub's only read of `society.state`"*. Measured with the instrument now published above, it is **4** plus the `.d.ts`; the `:670` citation pointed at the doc-comment rather than the getter at `:673`. The verdict and the MEDIUM severity are unchanged — the correction runs in this finding's own favour, since the cost argument it argues *up* gets stronger. The count had been asserted from a grep run early and not re-run as the finding grew around it; see §6.

One genuinely helpful window fact: `206dd004` armed the repo's first Rust CI (`web4-core` 193+4 tests green, matrix over four crate roots). The rename now has a machine-checkable blast gate it did not have at C168 — though the commit's own scope note is explicit that **CI green is not CI gating** (branch protection is steward-side). Auditor **MUST NOT** self-apply: the enum name is public API and the choice between renaming the Rust enum, re-labelling the four consumer surfaces (and regenerating the committed `.d.ts`), and amending the spec is an operator call.
...
- **The instrument-discipline lesson, added under review — and it turns the bullet above back on this pass's own tooling.** N1 originally published a consumer set of **2** and asserted `admin.rs:282` was *"the hub's only read of `society.state`"*. Measured, it is **4** plus a committed `.d.ts`. The verdict, the severity and the load-bearing claim (nothing keys authority off `society.state`) all survived; only the **count** was wrong — asserted from a grep run once, early, and not re-run as the finding grew around it. That is structurally identical to the mirror-set failure this same section diagnoses one bullet up (*"derived once, early … re-run rather than re-derived ever since"*), applied to the auditor's **own instrument** instead of the lineage's artifact set. It is also the **second consecutive pass** with a miscounted evidence cell: C282's `hub/` gate published a five-token cluster as "0" that measures 27. Both verdicts were right; both cells were wrong; **both cells are the ones a forward ruling tells the next auditor to re-grep** — which is precisely where a wrong number does damage, because the next pass cites the cell, not the review thread. The rule, which C282's own §B′ already demonstrated and neither cell inherited: **publish the instrument beside the number, and re-run it *after* the finding is written, not before.** Applied above in §3.1 and N1.
...
3. **N1 is reach, not a new defect** — if C168-N1 is still open, check first whether **the two WASM faces (`wasm.rs:673` getter, `wasm.rs:628` `summary()`), the two operator labels (`admin.rs:282` page, `main.rs:1154` CLI) and the committed `pkg/web4_trust_core.d.ts` declarations** are still there, and whether anything began keying authority off `society.state` (today: nothing does — all four consumers are display/serialization). Re-run the published instrument, do not re-use this count: `git grep -nE 'society\.state|inner\.state' -- '*.rs'`, tests excluded.
