...
Indexed as **web4** (42806 symbols, 90314 relationships, 300 flows). If tools warn of stale index, run `npx gitnexus analyze`.

## Rules
- **MUST** run `gitnexus_impact` before editing any symbol. Report blast radius.
- **MUST** run `gitnexus_detect_changes()` before committing.
- **MUST** warn user on HIGH/CRITICAL risk before proceeding.
- Use `gitnexus_query` to find execution flows (not grep). Use `gitnexus_context` for full symbol context.
...
After committing, re-index: `npx gitnexus analyze` (add `--embeddings` if previously used).
<!-- gitnexus:end -->
