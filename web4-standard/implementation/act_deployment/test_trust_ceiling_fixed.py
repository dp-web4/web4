#!/usr/bin/env python3
"""
Fixed Comprehensive Tests for Trust Ceiling Mitigation - Session #49

Uses correct API for receive_propagated_trust.

Fixed from Session #48 where 6/11 tests failed due to API signature mismatch.

Session: #49 (Autonomous Research - API Fix + Security Testing)
"""
...
def test_propagated_trust_has_ceiling():
    ...
    # propagation_path = [source_lct, ...intermediate hops..., observer_lct]
    # For 1 hop: [intermediary_lct, observer_lct]
    engine.receive_propagated_trust(
        ...
        propagation_path=[intermediary_lct, observer_lct]
    )
...
def test_ceiling_prevents_perfect_collusion():
    ...
    for i in range(5):
        ...
        # Path: colluder → observer (1 hop)
        engine.receive_propagated_trust(
            subject_lct=attacker_lct,
            source_lct=colluder_lct,
            trust_score=1.0,
            propagation_path=[colluder_lct, observer_lct]
        )
...
def test_sybil_amplification_prevented():
    ...
    for i in range(100):
        ...
        engine.receive_propagated_trust(
            ...
            trust_score=1.0,
            propagation_path=[sybil_lct, observer_lct]
        )
    ...
    assert trust <= 0.701, f"Ceiling should prevent Sybil amplification: {trust}"
...
def test_reputation_washing_prevented():
    ...
    for i in range(10):
        ...
        engine.receive_propagated_trust(
            subject_lct=new_attacker_lct,
            source_lct=endorser_lct,
            trust_score=1.0,
            propagation_path=[endorser_lct, observer_lct]
        )
...
def run_all_tests():
    ...
    print("=" * 70)
    print("Trust Ceiling Comprehensive Test Suite - Session #49 (FIXED)")
    print("=" * 70)
    print("\nUsing formal LCT library + correct API")
    print()
