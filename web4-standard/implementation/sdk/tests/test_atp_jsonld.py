...
class TestATPAccountJsonLd:
    ...
    def test_roundtrip(self):
        ...
        original = ATPAccount(available=500.0, locked=100.0, adp=400.0, initial_balance=1000.0)
    ...
    def test_from_plain_dict(self):
        ...
        doc = {"available": 500.0, "locked": 0.0, "adp": 100.0, "initial_balance": 600.0}
        acct = ATPAccount.from_jsonld(doc)
...
class TestTransferResultJsonLd:
    ...
    def test_context_and_type(self):
        """JSON-LD document has correct @context and @type."""
        result = TransferResult(fee=5.0, sender_balance=895.0, receiver_balance=100.0, actual_credit=100.0)
    ...
    def test_overflow_included(self):
        ...
        result = TransferResult(
            fee=5.0, sender_balance=895.0, receiver_balance=100.0, actual_credit=80.0, overflow=20.0
        )
        doc = result.to_jsonld()
    ...
    def test_roundtrip(self):
        ...
        original = TransferResult(
            fee=10.0, sender_balance=890.0, receiver_balance=200.0, actual_credit=200.0, overflow=0.0
        )
        doc = original.to_jsonld()
    ...
    def test_roundtrip_with_overflow(self):
        ...
        original = TransferResult(
            fee=5.0, sender_balance=950.0, receiver_balance=50.0, actual_credit=30.0, overflow=20.0
        )
        doc = original.to_jsonld()
        restored = TransferResult.from_jsonld(doc)
    ...
    def test_string_roundtrip(self):
        """to_jsonld_string() → from_jsonld_string() round-trips."""
        original = TransferResult(fee=2.5, sender_balance=947.5, receiver_balance=50.0, actual_credit=50.0)
        s = original.to_jsonld_string()
    ...
    def test_from_plain_dict(self):
        """from_jsonld() accepts plain dict without @context/@type."""
        doc = {"fee": 3.0, "sender_balance": 897.0, "receiver_balance": 100.0, "actual_credit": 100.0}
    ...
    def test_capped_transfer_serialization(self):
        ...
        result = transfer(sender, receiver, 100.0, fee_rate=0.05, max_balance=100.0)
        doc = result.to_jsonld()
...
class TestATPSchemaValidation:
    ...
    def _load_schema(self):
        ...
        try:
            ...
            schema_path = os.path.join(os.path.dirname(__file__), "..", "..", "..", "schemas", "atp-jsonld.schema.json")
    ...
    def test_account_validates(self):
        ...
        acct = ATPAccount(available=1000.0, locked=200.0, adp=300.0, initial_balance=1500.0)
    ...
    def test_transfer_with_overflow_validates(self):
        ...
        result = TransferResult(fee=5.0, sender_balance=950.0, receiver_balance=50.0, actual_credit=30.0, overflow=20.0)
        self._validate(result.to_jsonld())
