...
class TestAgentPlanJsonLd:
    ...
    def test_required_fields(self):
        ...
        assert len(doc["canonicalHash"]) == 64  # SHA-256 hex
    ...
    def test_steps_serialized(self):
        ...
        step = doc["steps"][0]
        assert step["id"] == "s1"
        assert step["mcp"] == "invoice.search"
    ...
    def test_step_requires_approval(self):
        plan = make_plan(steps=[
            PlanStep(step_id="s1", mcp_tool="a.get", requires_approval="if_amount > 100"),
        ])
        doc = plan.to_jsonld()
        assert doc["steps"][0]["requiresApproval"] == "if_amount > 100"
    ...
    def test_guards_structure(self):
        ...
        assert g["resourceCaps"]["rateLimit"] == "5/hour"
        assert g["humanApproval"]["mode"] == "conditional"
        assert g["humanApproval"]["autoThreshold"] == 50.0
        assert g["humanApproval"]["timeout"] == 7200
        assert g["humanApproval"]["fallback"] == "deny"
        assert g["expiresAt"] == "2026-12-31T23:59:59+00:00"
    ...
    def test_triggers_with_authorized(self):
        plan = make_plan(triggers=[
            Trigger(kind=TriggerKind.MANUAL, authorized=["lct:user:u1", "lct:user:u2"]),
        ])
        doc = plan.to_jsonld()
        assert doc["triggers"][0]["kind"] == "manual"
        assert doc["triggers"][0]["authorized"] == ["lct:user:u1", "lct:user:u2"]
    ...
    def test_roundtrip(self):
        ...
        assert restored.guards.law_hash == plan.guards.law_hash
        assert restored.guards.witness_level == plan.guards.witness_level
    ...
    def test_roundtrip_preserves_guards(self):
        ...
        assert restored.guards.resource_caps.rate_limit == "5/hour"
        assert restored.guards.human_approval.mode == ApprovalMode.CONDITIONAL
        assert restored.guards.human_approval.auto_threshold == 50.0
        assert restored.guards.human_approval.timeout == 7200
    ...
    def test_roundtrip_preserves_triggers(self):
        ...
        assert restored.triggers[0].expr == "0 * * * *"
...
class TestIntentJsonLd:
    ...
    def test_roundtrip(self):
        ...
        assert restored.step_id == intent.step_id
        assert restored.confidence == intent.confidence
        assert restored.risk_assessment == intent.risk_assessment
        assert restored.needs_approval == intent.needs_approval
        assert restored.proof.grant_id == intent.proof.grant_id
...
class TestDecisionJsonLd:
    ...
    def test_deny_decision(self):
        ...
        assert doc["rationale"] == "Too risky"
    ...
    def test_roundtrip(self):
        ...
        assert restored.rationale == dec.rationale
...
class TestExecutionRecordJsonLd:
    ...
    def test_failure_result(self):
        ...
        assert doc["result"]["output"]["error"] == "timeout"
    ...
    def test_roundtrip(self):
        ...
        assert restored.grant_id == rec.grant_id
        assert restored.law_hash == rec.law_hash
        ...
        assert restored.t3v3_delta == rec.t3v3_delta
        assert restored.witnesses == rec.witnesses
...
class TestSchemaCompliance:
    """Verify to_jsonld() output is valid against JSON Schema."""

    def test_agent_plan_no_additional_properties(self):
        plan = make_plan()
        doc = plan.to_jsonld()
        # All keys should be recognized schema fields
