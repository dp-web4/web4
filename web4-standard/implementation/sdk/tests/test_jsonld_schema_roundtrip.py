"""
B4: Schema-validated JSON-LD round-trip integration tests.

For every type with to_jsonld(), this test:
...
Covers all 9 JSON-LD schemas and 19 distinct @type values:
  LCT, AttestationEnvelope, T3Tensor, V3Tensor, R7Action,
  ATPAccount, TransferResult, AgentPlan, Intent, Decision,
  ExecutionRecord, EntityTypeInfo, EntityTypeRegistry,
  LevelRequirement, CapabilityAssessment, CapabilityFramework,
  DictionarySpec, TranslationResult, TranslationChain, DictionaryEntity
...
class TestDictionarySchemaRoundtrip:
    ...
    def test_translation_result_validates(self, schema):
        ...
        result = TranslationResult(
            content="heart attack (common term for myocardial infarction)",
            confidence=0.95,
            degradation=0.05,
            dictionary_lct_id="lct:web4:dictionary:med-common:abc",
        )
        doc = result.to_jsonld()
        validate(doc, schema)
    ...
    def test_translation_result_roundtrip(self, schema):
        ...
        result = TranslationResult(
            ...
            confidence=0.88,
            degradation=0.12,
            dictionary_lct_id="lct:web4:dictionary:legal-biz:def",
        )
        doc = result.to_jsonld()
        validate(doc, schema)
    ...
    def test_translation_chain_validates(self, schema):
        ...
        chain.add_step(
            ...
            dictionary_lct_id="lct:web4:dictionary:med-nurse:abc",
            confidence=0.9,
        )
        chain.add_step(
            source_domain="nursing",
            target_domain="common",
            dictionary_lct_id="lct:web4:dictionary:nurse-common:def",
            confidence=0.92,
        )
        doc = chain.to_jsonld()
        validate(doc, schema)
    ...
    def test_translation_chain_roundtrip(self, schema):
        ...
        chain.add_step(
            ...
            dictionary_lct_id="lct:web4:dictionary:med-sci:abc",
            confidence=0.95,
        )
        chain.add_step(
            source_domain="scientific",
            target_domain="common",
            dictionary_lct_id="lct:web4:dictionary:sci-common:def",
            confidence=0.88,
        )
        doc = chain.to_jsonld()
        validate(doc, schema)
        ...
        assert restored.cumulative_confidence == pytest.approx(chain.cumulative_confidence)
