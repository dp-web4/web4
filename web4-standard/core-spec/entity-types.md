...
| **Role** | (Role itself is an entity type; roles are first-class. Roles fill roles in a degenerate sense — see §3.2, "The Role Revolution," above.) A role MAY define a **Maintainer** for its own state, scoped to the role's MRH — `society-roles.md` §2.9. |
...
| **Device** | Validator (hardware-attested), Oracle (sensor-based), Recovery-Authority (TPM-bound recovery), Witness (attestation device), Maintainer (self-test and resource reporting within its own MRH) |
