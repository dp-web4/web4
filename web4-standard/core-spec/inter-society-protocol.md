...
- ~~**Exchange-rate discovery mechanisms**~~ — **PARTIALLY RESOLVED v0.1.3**: the cross-society LCT envelope (mcp-protocol.md §7.4) carries `atp_settlement.exchange_rate` as a referenced negotiated rate; the negotiation protocol primitives ride on standard MCP tool calls between Treasurer roles. Per-pair *strategy* for arriving at the rate (bilateral bargaining, market-derived, third-party arbitration) remains society-sovereign, consistent with the ATP form/substance split in §4.
