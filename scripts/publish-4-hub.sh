...
grants each contributor a royalty-free patent license under that contributor's
essential patent claims for its contributor version; that grant is not
restricted to non-commercial use. See [`PATENTS.md`](PATENTS.md) for scope and
terms. Separate commercial licensing is available for non-AGPL-compliant uses
...
# REFUSE to destroy a commit that exists only on the mirror.
#
# The mirror is derived and this script force-pushes, so a commit made directly
# on 4-hub is not merged by a republish — it is overwritten, silently, with no
# conflict for anyone to notice. That is not hypothetical: dp-web4/4-hub#1
# ("docs: consistency evidence at Hub scale PRD") was filed and merged against
# the mirror on 2026-09-02, and HUB's next supervisor fire republished 12
# minutes later and deleted the file. It survived only because GitHub still
# served the force-pushed sha. The README below has said "file PRs against
# dp-web4/web4" the whole time; a note is not a guard.
#
# filter-repo is deterministic: unchanged upstream commits rewrite to identical
# shas run over run, so "on the remote but not in the history we just built" is
# exactly the set this push would destroy. The generated README commit is this
# script's own and is legitimately replaced each run, so it is excluded by
# author AND subject rather than by author alone.
#
# A sha-only test is too strict in one direction: the remedy printed below says
# "land this content in web4 and re-run; every republish will then reproduce
# it" — but a direct mirror commit keeps its sha forever, so once its content
# HAD been ported (dp's 7799508 -> web4#836, 2026-09-05) the guard still refused
# and the only exit was the override. What a force-push destroys is not a sha,
# it is the mirror tip's CONTENT at the paths those commits touched. So for each
# candidate, compare the mirror tip's blob at every path it touched with the
# rebuilt HEAD's blob at the same path (same sha, or absent on both sides). All
# equal => nothing is lost, report it as reproduced and proceed. Any difference
# => a real orphan, refuse as before. Comparing the TIP rather than the commit's
# own blob is deliberate: a direct commit later superseded by another direct
# commit on the mirror is "lost" only if the tip's final state is.
echo "[4-hub] checking the mirror for commits that exist only there..."
if git ls-remote --exit-code "$MIRROR_REMOTE" main >/dev/null 2>&1; then
  git fetch -q "$MIRROR_REMOTE" main:refs/remotes/mirror/main
  orphans=""
  for c in $(git rev-list refs/remotes/mirror/main ^HEAD); do
    ae="$(git log -1 --format=%ae "$c")"
    subj="$(git log -1 --format=%s "$c")"
    if [ "$ae" = "noreply@metalinxx.io" ] && \
       [ "$subj" = "mirror: root README (generated by scripts/publish-4-hub.sh)" ]; then
      continue
    fi
    reproduced=1
    for p in $(git diff-tree --root --no-commit-id --name-only -r "$c"); do
      tip_blob="$(git rev-parse -q --verify "refs/remotes/mirror/main:$p" 2>/dev/null || true)"
      head_blob="$(git rev-parse -q --verify "HEAD:$p" 2>/dev/null || true)"
      if [ "$tip_blob" != "$head_blob" ]; then
        reproduced=0
        break
      fi
    done
    if [ "$reproduced" -eq 1 ]; then
      echo "[4-hub]   reproduced: $(git log -1 --format='%h %cs %an  %s' "$c")"
      echo "[4-hub]     every path it touched already has the mirror tip's content at HEAD — nothing lost"
      continue
    fi
    orphans="$orphans $c"
  done
  if [ -n "$orphans" ]; then
    echo "[4-hub] REFUSING to publish: the mirror carries commit(s) this force-push would DESTROY." >&2
    for c in $orphans; do
      echo "[4-hub]   $(git log -1 --format='%h %cs %an  %s' "$c")" >&2
    done
    echo "[4-hub]" >&2
    echo "[4-hub] The mirror is fully derived. Land this content in the web4 monorepo under a" >&2
    echo "[4-hub] mirrored path (hub/, web4-core/, web4-policy/, web4-trust-core/) and re-run;" >&2
    echo "[4-hub] every republish will then reproduce it instead of deleting it." >&2
    echo "[4-hub]" >&2
    echo "[4-hub] If you changed the --path list above, EVERY rewritten sha moves and this check" >&2
    echo "[4-hub] fires on the whole history — a false positive, in the safe direction. Confirm" >&2
    echo "[4-hub] that is what you are seeing, then re-run with ALLOW_MIRROR_HISTORY_REWRITE=1." >&2
    if [ "${ALLOW_MIRROR_HISTORY_REWRITE:-}" != "1" ]; then
      echo "[4-hub] Nothing has been pushed." >&2
      exit 1
    fi
    echo "[4-hub] ALLOW_MIRROR_HISTORY_REWRITE=1 — proceeding anyway, as instructed." >&2
  fi
else
  echo "[4-hub]   remote has no main yet (first publish) — nothing to destroy."
fi

echo "[4-hub] force-pushing mirror..."
git push --force -q "$MIRROR_REMOTE" HEAD:main

echo "[4-hub] done: pushed to $MIRROR_REMOTE"
